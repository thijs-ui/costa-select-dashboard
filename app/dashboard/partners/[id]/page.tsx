...
import { ArrowLeft, TrendingUp, CalendarDays, Euro, Handshake } from 'lucide-react'
...
export default function PartnerDetailPage() {
  ...
  const totalCommissie = filteredDeals.reduce((s, d) => s + (d.partner_commissie ?? 0), 0)
  ...
  if (!partner) return <div className="text-slate-400 text-sm p-8">Partner niet gevonden.</div>

  return (
    <div className="p-6 max-w-5xl mx-auto">
      {/* Header */}
      <div className="mb-6">
        <Link href="/partners" className="inline-flex items-center gap-1.5 text-sm text-gray-400 hover:text-gray-600 mb-4">
          <ArrowLeft size={14} /> Terug naar partners
        </Link>
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">{partner.naam}</h1>
              <p className="text-sm text-gray-400">{[partner.land, partner.email].filter(Boolean).join(' · ')}</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <KpiCard icon={<CalendarDays className="w-4 h-4" />} label="Afspraken" value={String(filteredAfspraken.length)} color="blue" />
        <KpiCard
              ...
              <tbody>
                {filteredDeals.map((d, i) => (
                  <tr key={d.id} className={`border-b border-gray-100 hover:bg-gray-50 ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/40'}`}>
                    ...
                    <td className="px-4 py-2.5 text-right font-semibold text-gray-900">{d.partner_commissie ? formatEuro(d.partner_commissie) : '—'}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
      ...
      {/* Afspraken tabel */}
      <div>
        <h2 className="text-sm font-semibold text-gray-700 mb-3">Afspraken via deze partner ({filteredAfspraken.length})</h2>
