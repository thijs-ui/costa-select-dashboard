...
export default function FunnelPage() {
  ...
  const filteredOpenDeals = pipedriveDeals.filter(d => d.status === 'open' && isInRange(d.add_time, range))
  const filteredSales = sales.filter(s => isInRange(s.datum_passering, range))

  // Afhandeling: altijd alle huidige deals, ongeacht periode
  ...
  // Per-regio funnel — regular pipelines only
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-6">
            <div className="space-y-3">
              {funnelSteps.map((step, i) => {
                ...
                const conv =
                  i === 1 ? pct(totalDeals, totalLeads) :
                  i === 3 ? pct(totalSales, totalDeals + totalAfhandeling) : null
                return (
                  <div key={step.label}>
                    {i > 0 && (
                ...
                <tbody>
                  {regioFunnels.map((r, i) => (
                    <tr key={r.regio} className={`border-b border-gray-100 hover:bg-gray-50 ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/40'}`}>
                      ...
                      <td className="px-4 py-2.5 text-right"><span className="inline-flex items-center justify-center min-w-[2rem] px-2 py-0.5 rounded-full text-xs font-semibold bg-purple-100 text-purple-700">{r.leads}</span></td>
                      <td className="px-4 py-2.5 text-right"><span className="inline-flex items-center justify-center min-w-[2rem] px-2 py-0.5 rounded-full text-xs font-semibold bg-blue-100 text-blue-700">{r.deals}</span></td>
                      <td className="px-4 py-2.5 text-right"><span className="inline-flex items-center justify-center min-w-[2rem] px-2 py-0.5 rounded-full text-xs font-semibold bg-green-100 text-green-700">{r.sales}</span></td>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                <tr className="bg-white">
                  <td className="px-4 py-2.5 font-medium text-gray-700">Alle regio&apos;s + afhandeling</td>
                  <td className="px-4 py-2.5 text-right"><span className="inline-flex items-center justify-center min-w-[2rem] px-2 py-0.5 rounded-full text-xs font-semibold bg-purple-100 text-purple-700">{totalLeads}</span></td>
                  <td className="px-4 py-2.5 text-right"><span className="inline-flex items-center justify-center min-w-[2rem] px-2 py-0.5 rounded-full text-xs font-semibold bg-blue-100 text-blue-700">{totalDeals + totalAfhandeling}</span></td>
                  <td className="px-4 py-2.5 text-right"><span className="inline-flex items-center justify-center min-w-[2rem] px-2 py-0.5 rounded-full text-xs font-semibold bg-green-100 text-green-700">{totalSales}</span></td>
                  <td className="px-4 py-2.5 text-right"><PctBadge value={pct(totalDeals + totalAfhandeling, totalLeads)} good={20} /></td>
                  <td className="px-4 py-2.5 text-right"><PctBadge value={pct(totalSales, totalDeals + totalAfhandeling)} good={30} /></td>
                  <td className="px-4 py-2.5 text-right"><PctBadge value={pct(totalSales, totalLeads)} good={10} /></td>
                </tr>
              </tbody>
            </table>
